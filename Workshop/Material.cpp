...
bool Material::doParametersAndUniformsMatch() const {
  ...
  std::vector<std::string> uniformNames = uniformInfos | vws::transform(&UniformInfo::name) | rng::to<std::vector>();
  std::vector<std::string> parameterNames;
  for (const auto& [pName, pVal] : parameters)
    parameterNames.emplace_back(pName);
  ...
  auto uniformsWithoutParameters = uniformInfos 
    | vws::filter([&](auto& ui) { return rng::find(parameterNames, ui.name) == parameterNames.end() && ui.offset == -1 && !Material::perMeshUniforms.contains(ui.name); }) 
    | rng::to<std::vector>();
  ...
  auto parametersWithoutUniforms = parameterNames | vws::filter([&](auto& pName) { return rng::find(uniformNames, pName) == uniformNames.end(); }) | rng::to<std::vector>();
